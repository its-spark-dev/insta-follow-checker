...
def save_to_csv(nonfollowers, filename="not_following_back.csv"):
    """
    Save a list of users who don't follow back to a CSV file.

    Args:
        nonfollowers (set): A set of Instaloader Profile objects not following back.
        filename (str): Output CSV filename. Defaults to 'not_following_back.csv'.
    """
    with open(filename, "w", newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Username"])  # CSV header
